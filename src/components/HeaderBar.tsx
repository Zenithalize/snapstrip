  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-purple-100 px-3 py-2 sm:px-4 sm:py-3 shadow-xs">
      <div className="max-w-6xl mx-auto flex items-center justify-between gap-2 sm:gap-4">
        ...
        >
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-2xl bg-gradient-to-tr from-purple-500 via-pink-400 to-cyan-300 p-0.5 shadow-sm group-hover:scale-105 transition-transform">
          ...
          <div>
            <h1 className="font-heading font-black text-base sm:text-xl tracking-tight bg-gradient-to-r from-purple-600 via-pink-500 to-cyan-500 bg-clip-text text-transparent flex items-center gap-1 leading-none">
              SnapStrip <Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-pink-400 fill-pink-400" />
            </h1>
            <p className="text-[9px] sm:text-[11px] font-medium text-purple-400">K-Pop Photobooth</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <button
            onClick={onOpenCoOp}
            className={`flex items-center gap-1.5 px-2.5 py-1.5 sm:px-3 sm:py-1.5 rounded-full text-xs font-bold transition-all shadow-xs ${
              isCoOpActive
          ...
          >
            ...
            <span>{isCoOpActive ? 'Co-Op Active' : '2-6 Player Co-Op'}</span>
          </button>
        </div>
      </div>
    </header>
